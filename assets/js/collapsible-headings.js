(() => {
  ...
  const configElement = document.getElementById('collapsible-headings-config');
  ...
  const collapsibleLevels = new Set(config.levels ?? []);
  const expandedLevels = new Set(config.expanded ?? []);
  const supportedLevels = new Set([2, 3, 4]);

  const wrapHeadings = (container, level) => {

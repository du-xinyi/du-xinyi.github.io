...
categories: [单片机]
tags: [单片机, openocd]
img_path: 'posts/2023-10-26-openocd/'
---
...
## Linux安装OpenOCD
直接使用`apt`安装的`openocd`版本很低，直接会导致大部分芯片无法使用，还是选择从源码编译，从[openocd](https://github.com/openocd-org/openocd.git)下载源码
...
[一键安装及配置脚本](https://github.com/du-xinyi/du-xinyi.github.io/releases/download/origin/openocd.sh)，将脚本置于`openocd`源码目录下即可使用